...
const fetchCategories = async () => {
  const response = await fetchApi('category', { method: 'GET' });
  categories.value = response.data;
};

const loadPostData = async () => {
  const postId = route.params.postId;
  const response = await fetchApi(`post/${postId}`, { method: 'GET' });
  post.value = { ...response.data, user_id: authService.getUserData().id };
  selectedCategories.value = response.data.categories.data.map(c => c.id);
};
...
const submitPost = async () => {
  ...
  {
    try {
      await fetchApi(`post/${post.value.id}`, {
      ...
      await router.push({ name: 'PostDetail', params: { postId: post.value.id } });
    } catch (err) {
      if (err instanceof Error) {
        errorMessages.value.general = err.message;
      } else {
        errorMessages.value.general = 'Une erreur inattendue est survenue. Merci de réessayer plus tard.';
      }
    }
  }
};
